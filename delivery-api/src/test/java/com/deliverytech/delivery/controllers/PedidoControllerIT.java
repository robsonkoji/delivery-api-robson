...
class PedidoControllerIT {
    ...
    void setup() throws Exception {
        ...
        produto1.setCategoria("Categoria Exemplo");
        ...
        produto2.setCategoria("Categoria Exemplo");
    ...
    void deveCriarPedidoComSucesso() throws Exception {
        ...
        mockMvc.perform(post("/api/pedidos")
            ...
            .andExpect(jsonPath("$.dados.restaurante.id", is(restaurante.getId().intValue())))
    ...
    void deveAtualizarStatusPedido() throws Exception {
        ...
        mockMvc.perform(patch("/api/pedidos/{id}/status", pedidoId)
                .header("Authorization", "Bearer " + token)
                .param("status", StatusPedido.CONFIRMADO.name())  // Corrigido valor do parâmetro
                .contentType(MediaType.APPLICATION_JSON))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.dados.status", is(StatusPedido.CONFIRMADO.name())))
            .andDo(print());
    }
...
void deveBuscarPedidosPorCliente() throws Exception {
    ...
    // Cria o pedido, espera 201 Created
    mockMvc.perform(post("/api/pedidos")
            .header("Authorization", "Bearer " + token)
            .contentType(MediaType.APPLICATION_JSON)
            .content(objectMapper.writeValueAsString(pedidoRequest)))
    ...
    // Busca os pedidos do cliente, espera 200 OK
    mockMvc.perform(get("/api/pedidos/cliente/{clienteId}", cliente.getId())
            .header("Authorization", "Bearer " + token)
            .accept(MediaType.APPLICATION_JSON))  // garante que aceita JSON
        .andDo(print())
